// Stack farm_need_v2 inside r2_f PC1 (scaling input := clamp(5+scaling−tempo)).
// No new weight. Hidden OFF, open ON, same seed. Does not persist axis-weights.json.
...
const RUN_ID = process.env.R2_FARM_RUN_ID ?? 'r2-farm-2026-09-21';
...
function main(): void {
  ...
  console.log(`r2_f_farm ${RUN_ID}: production rwr=${productionWeight}; honest 0; hidden OFF; open ON`);
  ...
  console.log(`farm_v2 replaces scaling *inside* PC1; no new coefficient\n`);
  ...
  try {
    ...
    const output = path.join(outDir, 'r2_f_farm-heroTable.json');
    spawnWorker({
      B0_SEED: String(SEED),
      B0_MATCHES: String(N_MATCHES),
      B0_WRITE_OUTPUT: '1',
      B0_OUTPUT_PATH: output,
      B0_SUMMARY_PATH: path.join(outDir, 'r2_f_farm-summary.json'),
  ...
  const pick = failR
    ? 'farm_v2 inside PC1 killed r. Keep r2_f. Isolated farm_v2 stays a footnote.'
    : falseFail
      ? 'Coverage gate vs r2_f is a false fail: r/MAE/tail improved. Take r2_f_farm as the new f.'
      : autoFail
        ? 'Coverage regression without a compensating r/tail win. Keep r2_f; do not stack farm_v2.'
        : rUp || tailBetter
          ? 'farm_v2 stacks on r2_f. New f = r2_f_farm.'
          : 'Neutral stack. Keep r2_f unless late_cores MAE clearly dropped.';

  const clusters: Record<string, { r2: number | null; farm: number | null }> = {};
  for (const [name, heroes] of Object.entries(CLUSTERS)) {
    clusters[name] = { r2: clusterMae(r2Table, heroes), farm: clusterMae(table, heroes) };
  }

  const dr = r - (r2.rFavReal ?? 0);
  console.log(`r2_f_farm: ${fmt(kpi)}  Δr vs r2_f=${dr >= 0 ? '+' : ''}${dr.toFixed(3)}`);
  console.log(pick);

  const lines = [
    `# farm_need_v2 stacked on r2_f — ${RUN_ID}`,
    '',
    'Hidden OFF, open ON, `realWinRateWeight=0`. Same seed=1 × 100k.',
    'PC1 scaling input := clamp(5 + scaling − tempo). Body-in-PC1 + explicit + RE=0 kept.',
    'No new coefficient. JSON not written.',
    '',
    '## KPI',
    '',
    ...
    `| r2_f_farm | ${kpi.rFavReal?.toFixed(3)} | ${dr >= 0 ? '+' : ''}${dr.toFixed(3)} | ${kpi.maePp.toFixed(2)} | ${kpi.coverage7Pct.toFixed(1)}% | ${kpi.flagged10} |`,
    '',
    pick,
    '',
    '## Cluster MAE п.п. (lower better)',
    '',
    `| Cluster | r2_f | r2_f_farm |`,
    `|---|---:|---:|`,
    ...Object.keys(CLUSTERS).map(
      (c) => `| ${c} | ${clusters[c].r2?.toFixed(1) ?? ''} | ${clusters[c].farm?.toFixed(1) ?? ''} |`,
    ),
    '',
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      { runId: RUN_ID, createdAt: new Date().toISOString(), seed: SEED, nMatches: N_MATCHES, naked, r2, kpi, clusters, pick, falseFail, autoFail },
      null,
      2,
    ) + '\n',
  );
