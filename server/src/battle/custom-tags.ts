...
export function blessingEffectsFor(
  ...
  if (!isTagDisabled('The Fundamentals') && fundamentalsCount >= 2) {
    const boost = fundamentalsBoostMagnitude(fundamentalsCount);
    for (const axis of fundamentalsTargetAxes(rawAxisAverages, fundamentalsCount)) {
      mulAxis(effects, axis, boost);
    }
  }
