...
- Skirmish Rate: 5%
- Camp Stacking: 5%
- Resource Efficiency: **вес 0 с 2026-09-21** (mid / early / late в `axis-weights.json`). Ось на радаре и в breakdown остаётся: damage per team-networth-share. В Total Score доля 0, потому что Eval берёт mid-веса из того же JSON, что и Battle. Раньше документ врал, что ось «выключена из Battle»: ключ молча жил с весом 0.5, а пропущенный ключ в `axisWeightForPhase` равен 1, не 0. Аудит R1.5: r(RE, realWR)=−0.117, сумма осей с RE хуже, чем без. Это не «информационная карточка по плану августа», это выключенный предиктор. Не возвращать вес без нового прогона.
- Pro Similarity: 5%
