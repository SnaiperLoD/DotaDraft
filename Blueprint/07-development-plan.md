...
- Hero positions: per-match GPM rank within the hero's own team via Explorer (`RANK() OVER (PARTITION BY match_id, team ORDER BY gold_per_min DESC)`), sampled over a recent match_id window (~last 2-3 months of public matches). Originally `lane_role` + `is_roaming`, replaced — that method couldn't distinguish a support standing static in the safe lane from the actual carry in it (see `10-tech-debt-backlog.md`, "`presumed_positions`").
